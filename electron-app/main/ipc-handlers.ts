...
async function writeToHooksQueue(
  drawingId: string,
  drawingName: string,
  elementCount: number,
...
const closeSignals = new Map<string, boolean>();
const mermaidConversions = new Map<string, { definition: string; timestamp: number }>();

/**
 * Register all IPC handlers for drawing operations
 * This replaces the HTTP API from the web-app version
 */
export function registerIpcHandlers(storage: DrawingStorage): void {

  // Load drawing
  ipcMain.handle('load-drawing', async (_event, drawingId: string) => {
  ...
  // Update drawing name
  ipcMain.handle('update-drawing-name', async (_event, drawingId: string, newName: string) => {
  ...
  // Save drawing
  ipcMain.handle('save-drawing', async (_event, drawingId: string, data: {
    elements: any[];
    appState: any;
  }) => {
    ...
    const existingElementIds = new Set(existing.elements.map((el: any) => el.id));
    ...
    const validElements = data.elements.filter((el: any) => {
      ...
      if (el.type === 'freedraw' && el.points) {
        const hasInvalidPoints = el.points.some((p: any) =>
          p === null || p[0] === null || p[1] === null
        );
        if (hasInvalidPoints) return false;
      }
    ...
    const markedElements = validElements.map((el: any) => {
  ...
  // Check listen status
  ipcMain.handle('listen-status', async (_event, drawingId: string) => {
    try {
      ...
      const isListening =
        listenState.drawingId === drawingId &&
  ...
  ipcMain.handle('collaborate', async (_event, drawingId: string, data: {
    elementCount: number;
    timestamp: number;
  }) => {
  ...
  // This bypasses the MCP server to avoid race conditions with multiple Claude instances
  ipcMain.handle('finished', async (_event, drawingId: string, data: {
    elementCount: number;
    timestamp: number;
  }) => {
  ...
  ipcMain.handle('collaborate-status', async (_event, drawingId: string) => {
  ...
  ipcMain.handle('close-signal', async (_event, drawingId: string) => {
  ...
  ipcMain.handle('mermaid-status', async (_event, drawingId: string) => {
  ...
  ipcMain.handle('screenshot-request', async (_event, drawingId: string) => {
  ...
  ipcMain.handle('screenshot-result', async (_event, drawingId: string, result: {
    success: boolean;
    base64?: string;
    filePath?: string;
    width?: number;
  ...
  ipcMain.handle('delete-listen-state', async (_event, drawingId: string) => {
  ...
  ipcMain.handle('export-to-file', async (event, options: {
    ...
    const filters: { [key: string]: Electron.FileFilter[] } = {
 ...
 */
export function setCloseSignal(drawingId: string): void {
 ...
 */
export function addMermaidConversion(drawingId: string, definition: string): void {
