...
export class DrawingStorage {
    ...
    async deleteDrawing(id) {
        const drawingPath = path.join(this.baseDir, "drawings", `${id}.excalidraw`);
    ...
    async listDrawings(opts = {}) {
        ...
        try {
            ...
            for (const file of metaFiles) {
                try {
                    const content = await fs.readFile(path.join(drawingsDir, file), "utf-8");
            ...
            if (opts.search) {
                ...
                drawings = drawings.filter((d) => d.name.toLowerCase().includes(query) ||
    ...
    async saveDrawing(drawing) {
        const filePath = path.join(this.baseDir, "drawings", `${drawing.id}.excalidraw`);
        const data = {
            type: "excalidraw",
            version: 2,
    ...
    async saveMetadata(drawing) {
        const metaPath = path.join(this.baseDir, "drawings", `${drawing.id}.meta.json`);
        const metadata = {
            id: drawing.id,
            name: drawing.name,
...
//# sourceMappingURL=DrawingStorage.js.map